<x-countrywide.header title="Blog | CountryWide Process" />
<x-countrywide.nav />
                    ...
                    <div class="card-body">
                        ...
                        <table id="customers2" class="table datatable">
                            <thead>
                                ...
                                @php
                                    $imgdata = explode("|", $item -> image);
                                @endphp
                                <tr>
                                    <td style="color: black;">{{ $loop -> index + 1 }}</td>
                                    <td style="color: black;">{{ $item -> name }}</span></td>
                                    <td style="color: black;"><?= htmlspecialchars_decode(Str::limit($item->details,200,$end='...')) ?></span></td>
                                    <td style="color: black;"><div class="img">
                                        <img src="{{ asset($item->image) }}" alt="" height="60px" weidth="40px">
                                    </div></span></td>
                                    <td>
                                    @foreach($pdf as $doc)    
                                        @if($doc ->blog_id == $item->id)
                                                <a href="{{ asset($doc->pdf_link) }}" target="_blank" class="border d-inline-block fs-6 fw-bolder p-2 rounded-3 shadow text-center text-black">
                                                    <i class="fa fa-file-pdf-o fs-2 text-danger" ></i><br/>{{$item-> Name}} </a>
